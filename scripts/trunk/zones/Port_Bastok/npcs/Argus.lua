...
RANK_POINTS_DIALOG = 1587;   -- You do not need to donate any more crystals ..      
LOW_MISSION_OFFSET = 889;    -- You can consult the ..                              
MISSION_6_1_OFFSET = 2004;   -- Go to Ore Street ..                                 
...
if (player:getNation() == 1) then
  if fetichism_active and (trade:getItemCount() == 4) and 
    ...
    if (player:getRank() == 1) then
      ...
      player:setVar("FetichismPort",1)
    else
      if (player:getVar("FetichismPort") == 1) then
  ...
  elseif forsaken_mines_active and (trade:getItemCount() == 1) and (trade:hasItemQty(ChunkOfGlocolite,1)) then
    if (player:getVar("ForsakenMinesPort" == 0)) then
      player:startEvent(Event(0x03F2)); -- first time this quest has been finished in this area.
      player:setVar("ForsakenMinesPort",1);
    else
