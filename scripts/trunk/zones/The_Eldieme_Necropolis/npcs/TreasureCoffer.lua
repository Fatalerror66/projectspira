...
-------------------------------------------
   treasureLVL = 63;
       ...
       zoneKey = 0x416; -- 1046
-- Loot
  treasureGilArray = {0.5,7590,25000};
 treasureGemsArray = {0.25,Chrysoberyl,Flourite,Moonstone,Sunstone};
treasureItemsArray = {0.25,VileElixir1};

--     AF related
     OldGauntlets = 0xD2;
    WizardsTonban = 0x3789;
...
function onTrade(player,npc,trade)
	...
	if    (player:getQuestStatus(3,51) == 1 and player:hasKeyItem(OldGauntlets) == 0) then		-- DRK old gauntlets
		og = 1;
	elseif(player:getQuestStatus(3,50) == 1 and player:hasKeyItem(OldGauntlets) == 0) then		-- PLD old gauntlets
		og = 1;
	elseif(player:getQuestStatus(3,48) == 1 and player:hasKeyItem(OldGauntlets) == 0) then		-- RDM old gauntlets
		og = 1;
	elseif(player:getQuestStatus(3,44) == 1 and player:hasKeyItem(OldGauntlets) == 0) then		-- WAR old gauntlets
		og = 1;
	end
	-- Checking if you can get AF item after opening AF2 quest.
	if    (player:getVar("KNOWS_ABOUT_BORGHERTZ") == 1 and player:getQuestStatus(2,67) >= 1 and player:getMainJob() == 4 and not(player:hasItem(WizardsTonban) == 1)) then			-- BLM has completed AF2
		blmlegs = 1;
	end

	if(og == 1 or blmlegs == 1)then
		questItemNeeded = 1;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			elseif(blmlegs == 1) then
				-- BLM AF legs
				player:addItem(WizardsTonban);
				player:specialMessage(ITEM_OBTAINED,WizardsTonban);
			else
			    -- Not AF
