...
RANK_POINTS_DIALOG = 1849; -- You do not need to donate any more crystals ..      
LOW_MISSION_OFFSET = 14;   -- You can consult the ..                              
MISSION_6_1_OFFSET = 2191; -- Go to Ore Street ..                                 
...
03F2 final for "To the Forsaken Mines" (with CS)
...
if (player:getNation() == 1) then
  if fetichism_active and (trade:getItemCount() == 4) and 
    ...
    if (player:getRank() == 1) then
      ...
      player:setVar("FetichismMetalworks",1)
    else
      if (player:getVar("FetichismMetalworks") == 1) then
  ...
  elseif forsaken_mines_active and (trade:getItemCount() == 1) and (trade:hasItemQty(ChunkOfGlocolite,1)) then
    if (player:getVar("ForsakenMinesMetalworks" == 0)) then
      player:startEvent(Event(0x03F2)); -- first time this mission has been finished in this area.
      player:setVar("ForsakenMinesMetalworks",1);
    else
