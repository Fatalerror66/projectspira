...
questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.478,5100,12450};
 treasureGemsArray = {0.087,Ametrine,Garnet,Goshenite,LightOpal,Onyx,Peridot,Sphene,Turquoise};
treasureItemsArray = {0.435,LifeBelt};
-----------------------------------
-- onTrigger Action
-----------------------------------
...
function onTrade(player,npc,trade)
    ...
    gil = trade:getGil();
	son = 0;
	FeiYinMagicTome = 0xB3;
	-- Sorcery of the North 
	if(player:getQuestStatus(0,83) == 1 and player:hasKeyItem(FeiYinMagicTome) == 0) then
		son = 1;
		questItemNeeded = son;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			if( son == 1) then
				player:addKeyItem(FeiYinMagicTome);
				player:specialMessage(KEYITEM_OBTAINED,FeiYinMagicTome);
			else
