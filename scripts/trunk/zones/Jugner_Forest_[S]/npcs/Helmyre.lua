...
if (license == 1 and level >= 20) then
	...
	--Dynamic pricing.
	timer = os.time();
	baseprice = CHOCOBO_PRICE_JUGNER_S;
	lastprice = getGlobalVar("chocoboRentPrice-Jugner");
	lasttime = getGlobalVar("chocoboRentTime-Jugner");
	decay = CHOCOBO_PRICE_JUGNER_S_DECAY;
	decaytime = CHOCOBO_PRICE_JUGNER_S_DECAY_TIME;
	...
	setGlobalVar("chocoboRentOffer-Jugner",price);
	event0xd0:setParams(price,player:getCurrGil()); 
...
if (count == 1 and item1 and level >= 20 and license) then
	...
	event0xd0 = Event(0xd0);
	event0xd0:setParams(0xffffffff);
	player:startEvent(event0xd0);
	...
	player:setPos(-120,0,-155,0x40,0x52);
end
end;  
...
--print("CSID:",csid);
--print("OPTION:",option);
if (csid == 0xd0 and option == 0) then
	flux = CHOCOBO_PRICE_JUGNER_S_FLUX;
	price = getGlobalVar("chocoboRentOffer-Jugner");
	player:removeGil(price);
	setGlobalVar("chocoboRentPrice-Jugner",price+flux);
