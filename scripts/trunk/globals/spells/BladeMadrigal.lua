...
--
--	http://wiki.ffxiclopedia.org/wiki/Madrigal
--------------------------------------
-- Includes
...
function OnSpellCast(caster,target,spell)
	...
	-- Defines
	name = EFFECT_MADRIGAL; --Actually is type, but type is a keyword in LUA.
	tic = 0;
	duration = 120;
	...
	power = 13 + math.floor(caster:getSkill(SINGING_SKILL)/16);
	...
	if (instrument == Gjallarhorn or (instrument == 18341 and isInDynamis(caster))) then
		power = power + 8;
	elseif (instrument == 17348) then --Traversiere
		power = power + 4;
	elseif (instrument == 17375 or instrument == 17845) then --Traversiere +1 or Traversiere +2
		power = power + 8;
	end
