...
--
--	http://wiki.ffxiclopedia.org/wiki/Minuet
--------------------------------------
-- Includes
...
function OnSpellCast(caster,target,spell)
	...
	-- Defines
	name = EFFECT_MINUET; --Actually is type, but type is a keyword in LUA.
	tic = 0;
	duration = 120;
	...
	power = 53 + math.floor(caster:getSkill(SINGING_SKILL)/10);
	...
	if (instrument == Gjallarhorn or (instrument == 18341 and isInDynamis(caster))) then
		power = power + 5;
	elseif (instrument == 17344) then --Cornette
		power = power + 3;
	elseif (instrument == 17369 or instrument == 17846) then --Cornette +1 or Cornette + 2
		power = power + 5;
	end
