...
--	Author: Tenjou
-- 	Wind Carol II
-- 	Increases elemental resistance for party members within area of effect.
--
--	http://wiki.ffxiclopedia.org/wiki/Carol
--------------------------------------
-- Includes
...
function OnSpellCast(caster,target,spell)
	...
	-- Defines
	name = EFFECT_CAROL; --Actually is type, but type is a keyword in LUA.
	tic = 0;
	duration = 120;
	...
	power = 2*(4 + math.floor(caster:getSkill(SINGING_SKILL)/8));
	...
	if (instrument == Gjallarhorn or (instrument == 18341 and isInDynamis(caster))) then
		power = power + 10;
	elseif (instrument == 17361) then --Crumhorn
		power = power + 5;
	elseif (instrument == 17377 or instrument == 17847) then --Crumhorn +1 or Crumhorn +2
		power = power + 10;
	end
	
	--Encode the element into the carol's power.
	element = 4; --See effects/Carol.lua
