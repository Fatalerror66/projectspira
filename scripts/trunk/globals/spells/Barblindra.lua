...
--	Author: Tenjou
-- 	Barblind
-- 	Increases caster's resistance against blindness.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARSLEEP,EFFECT_BARVIRUS do
		...
		if (barstatus ~= nil and (BARSTATUS_OVERWRITE == 1 or (BARSTATUS_OVERWRITE == 0 and i == EFFECT_BARBLIND))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARBLIND,resist,0,duration,FLAG_DISPELABLE);
