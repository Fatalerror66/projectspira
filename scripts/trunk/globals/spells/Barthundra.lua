...
--	Author: Tenjou
-- 	Barthundra
-- 	Increases caster's resistance against thunder.  Afflatus Solace: Grants MDB against thunder.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARFIRE,EFFECT_BARWATER do
		...
		if (barelement ~= nil and (BARELEMENT_OVERWRITE == 1 or (BARELEMENT_OVERWRITE == 0 and i == EFFECT_BARTHUNDER))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARTHUNDER,resist,0,duration,FLAG_DISPELABLE);
	target:updateEnmity(caster,80,1);
	final = EFFECT_BARTHUNDER;
