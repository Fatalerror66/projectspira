...
--	Author: Tenjou
-- 	Archer's Prelude
-- 	Increases target's ranged accuracy.
--
--	http://wiki.ffxiclopedia.org/wiki/Prelude
--------------------------------------
-- Includes
...
function OnSpellCast(caster,target,spell)
	...
	-- Defines
	name = EFFECT_PRELUDE; --Actually is type, but type is a keyword in LUA.
	tic = 0;
	duration = 120;
	...
	if (instrument == Gjallarhorn or (instrument == 18341 and isInDynamis(caster))) then
		power = power + 8;
	elseif (instrument == 17350) then --Angel's Flute
		power = power + 4;
	elseif (instrument == 17378) then --Angel's Flute +1
		power = power + 8;
	end
