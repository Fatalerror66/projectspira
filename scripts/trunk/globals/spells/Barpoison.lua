...
--	Author: Tenjou
-- 	Barpoison
-- 	Increases caster's resistance against poison.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARSLEEP,EFFECT_BARVIRUS do
		...
		if (barstatus ~= nil and (BARSTATUS_OVERWRITE == 1 or (BARSTATUS_OVERWRITE == 0 and i == EFFECT_BARPOISON))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARPOISON,resist,0,duration,FLAG_DISPELABLE);
