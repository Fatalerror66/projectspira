...
--	Author: Tenjou
-- 	Barblizzard
-- 	Increases caster's resistance against ice.  Afflatus Solace: Grants MDB against ice.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARFIRE,EFFECT_BARWATER do
		...
		if (barelement ~= nil and (BARELEMENT_OVERWRITE == 1 or (BARELEMENT_OVERWRITE == 0 and i == EFFECT_BARBLIZZARD))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARBLIZZARD,resist,0,duration,FLAG_DISPELABLE);
