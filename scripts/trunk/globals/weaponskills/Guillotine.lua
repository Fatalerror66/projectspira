...
-- Guillotine - 4 HIT
-- STR:25% MND:25%
-- fTP Multipliers (100/200/300) = 0.875,0.875,0.875
-- Author: Kegsay
---------------------------
-- Includes
...
function onUseWeaponSkill(attacker, target, wsID)
	...
	local base = attacker:getBaseDmg() + fSTR + 
		(attacker:getStat(1) * 0.25 + attacker:getStat(6) * 0.25) * alpha; --D+fSTR+WSC
	...
	--Random num to work out if hit
	--Fourth hit, fTP goes to 1.0 so just using base instead of of dmg.
	local double sechit = math.random();
	if (sechit<=hitrate) then
	--print("Hit");
	finaldmg = finaldmg+base*pdif;
