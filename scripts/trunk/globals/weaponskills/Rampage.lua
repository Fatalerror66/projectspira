...
-- Rampage - 5 HIT
-- STR:30%
-- fTP Multipliers (100/200/300) = 0.5,0.5,0.5
-- Crit hit rate VS TP is unknown so has been estimated.
-- Linear, 100%TP=10%crithitrate / 200%=30% / 300%=50% 
-- Author: Kegsay
---------------------------
-- Includes
...
function onUseWeaponSkill(attacker, target, wsID)
	...
	local int crithitrate = ((attacker:getTP()/5)-10)/100;
	...
	--working out hit rate for 2nd, 3rd and 4th and 5th hits
	...
	--Random num to work out if hit#5
	local double penthit = math.random();
	local double penthitc = math.random();
	if (penthit<=hitrate) then
	--print("Hit.");
	if (penthitc<=crithitrate) then
	--print("Critical Hit!");
	finaldmg = finaldmg + base*cpdif;
	--print("final damage so far: ",finaldmg);
	end
...
---------------------------------------------
-- Axe Weapon Skill Quest
---------------------------------------------
	PickofTrials = 0x460D; -- Trial Axe
	if(attacker:getEquip(1) == PickofTrials and attacker:getQuestStatus(3,59) == 1 and attacker:getVar("AxeWeaponSkillPoints") < 300 and givesEXP(target,attacker) and finaldmg > 0) then
			attacker:setVar("AxeWeaponSkillPoints",attacker:getVar("AxeWeaponSkillPoints")+1*WEAPON_SKILL_POINTS);		
	end
--------------------------------------------
	return finaldmg;
end
