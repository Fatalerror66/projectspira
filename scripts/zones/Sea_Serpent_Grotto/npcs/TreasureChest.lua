...
questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.962,3360,7200};
 treasureGemsArray = {0.038,Ametrine,Goshenite,Garnet,LightOpal,Sphene,Peridot,Onyx,Turquoise};
...
function onTrade(player,npc,trade)
	...
	Map = 0x1AA;
	-- A Test of True Love 
	if(player:getQuestStatus(1,62) == 1 and player:hasKeyItem(LEphemere) == 0) then
		totl = 1;
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			elseif(totl == 1) then
				...
				player:specialMessage(KEYITEM_OBTAINED,LEphemere);			
			else
