...
local wildcatc = testflag(tonumber(wildcatcount),0x2000);
--print("Wildcat: ",wildcat);
--print("WildcatCount: ",wildcatcount);
--print("WildcatC: ",wildcatc);
if (wildcat == 1 and wildcatc == false) then
	player:startEvent(Event(0x3aa));
	player:setVar("lureOfTheWildcat-W-Counter",wildcatcount+0x2000);
else
	player:startEvent(Event(0x240));
end 
end; 
