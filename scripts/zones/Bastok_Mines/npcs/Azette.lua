...
if (license == 1 and level >= 15) then
	...
	if (chocoquest >= (1 - CHOCOBO_RIDING_GAME_BASTOK) and os.time() >= (chocoquesttime+604800)) then
		...
		event0x3d:setParams(0xfffffffd,0,0,nation);
	else
		...
		event0x3d:setParams(price,player:getCurrGil(),lvlblock); 
	end
	player:startEvent(event0x3d);
else
...
if (count == 1 and item1) then
	...
	event0x3d = Event(0x3d);
	event0x3d:setParams(0xffffffff);
	player:startEvent(event0x3d);
