...
questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.8,5800,9600};
 treasureGemsArray = {0.2,Sphene,Peridot};
...
function onTrade(player,npc,trade)
    ...
    gil = trade:getGil();
	psm = 0;
	PieceofRippedFloorplans = 0x25D;
	-- Paradise, Salvation, and Maps 
	if(player:getQuestStatus(4,68) == 1 and player:hasKeyItem(PieceofRippedFloorplans) == 0) then
		psm = 1;
		questItemNeeded = psm;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			if(psm == 1) then
				player:addKeyItem(PieceofRippedFloorplans);
				player:specialMessage(KEYITEM_OBTAINED,PieceofRippedFloorplans);
			else
