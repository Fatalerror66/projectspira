-----------------------------------
--	Author: Psyclosis
--	CELESTIAL AVATAR: Titan Prime
--	Fights players in the battlefield "Trial by Earth."
...
function OnMobDeath(mob, killer)
	timeElapsed = os.time() - getGlobalVar("TRIAL_BY_EARTH_ENTER_TIME");
	event = Event(0x7D01);
	record = getGlobalVar("TRIAL_BY_EARTH_RECORD");
	if (timeElapsed < record) then
		setGlobalVar("TRIAL_BY_EARTH_RECORD",timeElapsed);
	end	 
	setGlobalVar("TRIAL_BY_EARTH_ENTER_TIME",0);
