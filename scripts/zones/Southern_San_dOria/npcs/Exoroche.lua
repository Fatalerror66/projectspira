...
--   
-- By: Mana
-- Quest "Father and Son" by ReaperX
...
if (player:getVar("father_sun_status") == 1) then
  player:startEvent(Event(0x21E));
elseif (player:getVar("father_sun_status") == 2) then
  player:showText(npc,1130)
else
  player:showText(npc,2456)
end;
...
--print("Event ID =  ",csid);
if (csid == 0x21E) then
  player:setVar("father_sun_status",2);
end;
end;
