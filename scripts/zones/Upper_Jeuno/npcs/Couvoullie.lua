...
if (license == 1 and level >= 20) then
	...
	--Dynamic pricing.
	timer = os.time();
	baseprice = CHOCOBO_PRICE_UPPER_JEUNO;
	lastprice = getGlobalVar("chocoboRentPrice-UpperJeuno");
	lasttime = getGlobalVar("chocoboRentTime-UpperJeuno");
	decay = CHOCOBO_PRICE_UPPER_JEUNO_DECAY;
	decaytime = CHOCOBO_PRICE_UPPER_JEUNO_DECAY_TIME;
	...
	setGlobalVar("chocoboRentOffer-UpperJeuno",price);
...
if (csid == 0x2713 and option == 0) then
	flux = CHOCOBO_PRICE_UPPER_JEUNO_FLUX;
	price = getGlobalVar("chocoboRentOffer-UpperJeuno");
	player:removeGil(price);
	setGlobalVar("chocoboRentPrice-UpperJeuno",price+flux);
