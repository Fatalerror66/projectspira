...
-------------------------------------------
   treasureLVL = 44;
       ...
       zoneKey = 0x40B; -- 1035
-- Loot
  treasureGilArray = {0.417,3200,6320};
 treasureGemsArray = {0.083,Ametrine,Garnet,LightOpal,Turquoise,Goshenite,Sphene,Peridot,Sardonyx};
treasureItemsArray = {0.50,ElectrumHairpin};
-----------------------------------
-- onTrigger Action
-----------------------------------
...
if( player:getVar("ScatteredintoShadow") == 2 and player:hasItem(BeastCollar) == 0)then
	sis = 1;
	questItemNeeded = sis;
end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			if(sis == 1)then
				player:addKeyItem(BeastCollar);
				player:specialMessage(ITEM_OBTAINED,BeastCollar);
			else			
