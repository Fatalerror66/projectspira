...
if (rainbow == 1 and ruby == 1) then
	...
	smncs = -1; --Should be possibly 0x1, new NPC capture required.
...
if (csid == 0x30) then
player:removeKeyItem(0x3b);
player:removeKeyItem(0x3c);
player:removeKeyItem(0x3d);
player:removeKeyItem(0x3e);
player:removeKeyItem(0x3f);
player:removeKeyItem(0x40);
--must replace with packet 2a
sendTellMsg("[pXI]Promathia",player:getName(),"The Mana Orbs have been stolen!");
player:setVar("theHeartOf-CardianCS",1);
end
end;
...
setGlobalVar("WindHeadstoneActive", os.time());
