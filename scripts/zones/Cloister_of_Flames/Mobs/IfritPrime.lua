-----------------------------------
--	Author: Psyclosis
--	CELESTIAL AVATAR: Ifrit Prime
--	Fights players in the battlefield "Trial by FIRE."
...
function OnMobDeath(mob, killer)
	timeElapsed = os.time() - getGlobalVar("TRIAL_BY_FIRE_ENTER_TIME");
	event = Event(0x7D01);
	record = getGlobalVar("TRIAL_BY_FIRE_RECORD");
	if (timeElapsed < record) then
		setGlobalVar("TRIAL_BY_FIRE_RECORD",timeElapsed);
	end	 
	setGlobalVar("TRIAL_BY_FIRE_ENTER_TIME",0);
