...
--	Allows qualified players to enter the battlefield to fight Ramuh Prime.
--
...
function onTrigger(player,npc)
	...
	---- 0: None
	---- 1: Trial by Lightning
	---- 2: Carbuncle Debacle
	---- 4: Trial-Size Trial by Lightning
	---- 8: Waking the Beast
	...
	if (npc:getXPos() >= 530 and npc:getXPos() <= 550 and npc:getZPos() >= 490 and npc:getZPos() <= 510) then --Entry crystal.
...
function onEventSelection(player,csid,option)
	...
	if (csid == 0x7d00 and option == 0) then
		record = getGlobalVar("TRIAL_BY_LIGHTNING_RECORD");
...
function onEventFinish(player,csid,option)
	...
	if (csid == 0x7D00 and option == 100) then --Enter the BC.
		...
		zone0xca = Zone(0xca);
		zone0xca:spawnMob("RamuhPrime",500,42,-369,0x60,0);
		setGlobalVar("TRIAL_BY_LIGHTNING_ENTER_TIME",os.time());
	end	
end;