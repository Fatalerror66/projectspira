-----------------------------------
--	Author: Psyclosis
--	CELESTIAL AVATAR: Shiva Prime
--	Fights players in the battlefield "Trial by Ice."
...
function OnMobDeath(mob, killer)
	timeElapsed = os.time() - getGlobalVar("TRIAL_BY_ICE_ENTER_TIME");
	event = Event(0x7D01);
	record = getGlobalVar("TRIAL_BY_ICE_RECORD");
	if (timeElapsed < record) then
		setGlobalVar("TRIAL_BY_ICE_RECORD",timeElapsed);
	end	 
	setGlobalVar("TRIAL_BY_ICE_ENTER_TIME",0);
