...
--	Author: Tenjou
-- 	Barsilence
-- 	Increases caster's resistance against silence.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARSLEEP,EFFECT_BARVIRUS do
		...
		if (barstatus ~= nil and (BARSTATUS_OVERWRITE == 1 or (BARSTATUS_OVERWRITE == 0 and i == EFFECT_BARSILENCE))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARSILENCE,resist,0,duration,FLAG_DISPELABLE);
