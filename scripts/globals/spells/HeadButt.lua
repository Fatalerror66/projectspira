...
--  DOES NOT DO KNOCKBACK ANIMATION
...
WSC = math.floor(((STR*0.2)+(INT*0.2))*alpha);
...
if (chance<=0.95) then --this will need to be changed to something more accurate!
	...
	if (stun ~= nil) then --stun is already on monster! Do not apply. 
		...
		target:addStatusEffect(EFFECT_STUN,1,0,8); --duration set to 8, will need to be modified depending on resistance.
	end
end
