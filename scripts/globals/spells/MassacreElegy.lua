...
--	Author: Tenjou
-- 	Battlefield Elegy
-- 	Decreases target's attack speed.
--
...
function OnSpellCast(caster,target,spell)
	...
	-- Defines
	name = EFFECT_ELEGY; --Actually is type, but type is a keyword in LUA.
	tic = 0;
	duration = 120;
	...
	if (instrument == Gjallarhorn or (instrument == 18341 and isInDynamis(caster))) then
		power = power + 2;
	elseif (instrument == 17352) then --Horn
		power = power + 1;
	elseif (instrument == 17371) then --Horn +1
		power = power + 2;
	end
	...
	for i = 1368,1469 do --Covers all Bard songs.
		...
		if (curreffect ~= nil) then --Found a song on the target.
			if (curreffect:getType() == songid) then --The song found is this song; erase it and stop checking.
				...
				do break end
			elseif (curreffect:getPower() == name) then --Effect is another elegy; erase it and stop checking.
				target:removeStatusType(songid); --Actual effect.
				target:removeStatusType(name); --Icon.
				do break end
			else
	...
	target:addStatusEffect(songid,name,tic,duration,FLAG_ERASABLE); --Add effect to track this song.
