---------------------------
-- Steel Cyclone - one hit Weapon Skill
-- STR:50% VIT:50%
-- fTP Multipliers (100/200/300) = 1.5,1.75,3
-- Author: Kegsay
---------------------------
-- Includes
...
function onUseWeaponSkill(attacker, target, wsID)
	...
	local base = attacker:getBaseDmg() + fSTR + 
		(attacker:getStat(1) * 0.5 + attacker:getStat(3) * 0.5) * alpha; --D+fSTR+WSC
	...
	ftp = attacker:getTP()/80 - 0.75;
	end
	
	--Obtaining cRatio. Also accounts for Steel Cyclones Attk Bonus.
	local double ratio = (attacker:getAtt()*(5/3))/target:getDef();
	--Level penalty...
