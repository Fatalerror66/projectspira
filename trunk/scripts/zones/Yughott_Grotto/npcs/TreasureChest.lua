...
questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.455,800,2350};
 treasureGemsArray = {0.09,Amber,Amethyst,LapisLazuli,Sardonyx,Tourmaline};
treasureItemsArray = {0.455,CougarBaghnakhs};
-----------------------------------
-- onTrigger Action
-----------------------------------
...
-----------------------------------
function onTrigger(player,npc)
player:specialMessage(CHEST_LOCKED,GhelsbaChestKey);
end;
...
function onTrade(player,npc,trade)
count = trade:getItemCount();
  key = trade:hasItemQty(GhelsbaChestKey,1);
...
if ((key or sk or lk or ttk) and count and gil == 0) then  -- Player traded a key and zero gil.
	...
	illu  = getGlobalVar("TreasureChestGhelsbaYughottGrotto");        -- SE impleted this in order to prevent coffer farming. Noone in the same area can open more than 1 coffer per hour except for AF and maps.
	if(os.time() <= weak) then
		player:specialMessage(CHEST_WEAK);
		success = -1;
	elseif( illu > os.time()) then
		player:specialMessage(CHEST_ILLUSION);
	...
	if(not(success == -2)) then
		...
		-- Succeded to open the coffer
			player:specialMessage(CHEST_UNLOCKED);
			setGlobalVar("TreasureChestGhelsbaYughottGrotto",os.time() + math.random(CHEST_MIN_ILLUSION_TIME,CHEST_MAX_ILLUSION_TIME));
