...
function onTrigger(player,npc)
	orb = player:getVar("groaningpond")
	...
	if(player:hasKeyItem(0x83) ==1 and orb == 0) then --checks for White Orb
		...
		player:setVar("groaningpond",1) --Tells server that this pond has been 'used',so the player cant spam the same pool
