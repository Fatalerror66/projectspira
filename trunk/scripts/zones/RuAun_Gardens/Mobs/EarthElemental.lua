...
function OnMobDeath(mob,killer)
  --Check if FoV ID 146 is active
  local fov = killer:getVar("fov_regimeid");
  --print("Working....");
  
  if (fov==146) then --have FoV active
	...
	if (numkilled<numneeded) then --increment number killed!
		...
		if (numkilled==numneeded) then
			...
			local numneeded2 = killer:getVar("fov_numneeded3");
			local numkilled2 = killer:getVar("fov_numkilled3");
			local numneeded3 = killer:getVar("fov_numneeded1");
			local numkilled3 = killer:getVar("fov_numkilled1");
