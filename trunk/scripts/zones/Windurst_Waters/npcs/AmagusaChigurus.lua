...
local wildcatc = testflag(tonumber(wildcatcount),0x1000);
--print("Wildcat: ",wildcat);
--print("WildcatCount: ",wildcatcount);
--print("WildcatC: ",wildcatc);
if (wildcat == 1 and wildcatc == false) then
	player:startEvent(Event(0x3a9));
	player:setVar("lureOfTheWildcat-W-Counter",wildcatcount+0x1000);
else
	player:startEvent(Event(0x232));
end 
end; 
