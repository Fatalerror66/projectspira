...
--  Equation of the door is X = 195.6-.7776*Z. This is needed to check whether we are on inside or outside.
...
X = player:getXPos();
Z = player:getZPos();
if (X > 195.6-.7776*Z) or (player:hasKeyItem(MoongatePass) == 1) then -- we are on the inside of the moon gate.
  npc:setState(20);
else
