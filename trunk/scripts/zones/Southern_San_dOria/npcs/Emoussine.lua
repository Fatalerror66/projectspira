...
if (license == 1 and level >= 15) then
	...
	if (chocoquest >= (1 - CHOCOBO_RIDING_GAME_SANDORIA) and os.time() >= (chocoquesttime+604800)) then
		...
		if (nation == 0) then
			nation = math.random(1,2);
		elseif (nation == 6) then
			nation = math.random(3,5);
		end
		event0x258:setParams(0xfffffffd,0,0,nation);
	else
		...
		event0x258:setParams(price,player:getCurrGil(),lvlblock); 
	end
	player:startEvent(event0x258);
else
	player:startEvent(Event(0x25b));
end
end; 
...
if (count == 1 and item1) then
	...
	event0x258 = Event(0x258);
	event0x258:setParams(0xffffffff);
	player:startEvent(event0x258);
...
--print("CSID:",csid);
--print("OPTION:",option);
if (csid == 0x258 and option == 0) then
