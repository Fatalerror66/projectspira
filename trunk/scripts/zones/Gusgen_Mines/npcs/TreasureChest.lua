...
SEServerfirstStart = 1009810800;
   ...
   questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.387,1980,3600};
 treasureGemsArray = {0.258,Tourmaline,ClearTopaz,Amber,Amethyst,LightOpal,Onyx};
treasureItemsArray = {0.355,FrostShield,ReplicaMaul};
-----------------------------------
-- onTrigger Action
-----------------------------------
...
--print("Current Race +1: ",CurrentRace+1); -- debug
--print("Player race: ", player:getRace()); -- debug
--print("Current Dungeon: ",CurrentDungeon); -- debug
if(player:getQuestStatus(3,42) == 1 and player:getRace() == CurrentRace+1 and player:hasKeyItem(MagicalPattern) == 0 and CurrentDungeon == 1) then
	tgt = 1;
	questItemNeeded = tgt;
end
