...
--	Author: Tenjou
-- 	Barparalyze
-- 	Increases caster's resistance against paralysis.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARSLEEP,EFFECT_BARVIRUS do
		...
		if (barstatus ~= nil and (BARSTATUS_OVERWRITE == 1 or (BARSTATUS_OVERWRITE == 0 and i == EFFECT_BARPARALYZE))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARPARALYZE,resist,0,duration,FLAG_DISPELABLE);
