...
--	Author: Tenjou
-- 	Barwater
-- 	Increases caster's resistance against water.  Afflatus Solace: Grants MDB against water.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARFIRE,EFFECT_BARWATER do
		...
		if (barelement ~= nil and (BARELEMENT_OVERWRITE == 1 or (BARELEMENT_OVERWRITE == 0 and i == EFFECT_BARWATER))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARWATER,resist,0,duration,FLAG_DISPELABLE);
