...
-- Savage Blade - 2 HIT
-- STR:30%  MND:50%
-- fTP Multipliers (100/200/300) = 1, 1.75, 3.5 
-- Author: Kegsay
---------------------------
-- Includes
...
function onUseWeaponSkill(attacker, target, wsID)
	...
	local base = attacker:getBaseDmg() + fSTR + 
		(attacker:getStat(1) * 0.3 + attacker:getStat(6) * 0.5) * alpha; --D+fSTR+WSC
		
	--Applying fTP multiplier
	if (attacker:getTP()<200) then
	ftp = (attacker:getTP()/133.33)+0.25;
	end
	if (attacker:getTP()>199) then
	ftp = (attacker:getTP()/57.14)-1.75;
	end
	...
	--Capping cRatio (note: Savage Blade cannot critical.)
	if (ratio<0) then
	ratio = 0;
	end
	if (ratio>2) then
	...
	--Savage Blade cannot crit without Sneak Attack!
