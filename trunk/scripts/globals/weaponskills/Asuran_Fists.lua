---------------------------
-- Asuran Fists (Beta) - 8 HIT - Accuracy varies with TP
-- STR:10%  VIT:10%
...
function onUseWeaponSkill(attacker, target, wsID)
	...
	--Capping cRatio (note: Swift Blade cannot critical.)
	if (ratio<0) then
	ratio = 0;
	end
	if (ratio>2) then
	...
	--working out hit rate for other hits
...
---------------------------------------------
-- Hand-to-Hand Weapon Skill Quest (the quest idea here is wrong and needs to be fixed)
---------------------------------------------
	KnucklesfTrials = 0x41FC; -- Knuckles of Trials(17507)
	if(attacker:getEquip(1) == KnucklesSpearofTrials and attacker:getQuestStatus(0,98) == 1 and attacker:getVar("Hand-to-HandWeaponSkillPoints") < 300 and givesEXP(target,attacker) and finaldmg > 0) then
			attacker:setVar("Hand-to-HandWeaponSkillPoints",attacker:getVar("Hand-to-HandWeaponSkillPoints")+1*WEAPON_SKILL_POINTS);		
	end
--------------------------------------------
	return finaldmg;
end
