...
function onUseWeaponSkill(attacker, target, wsID)
	...
	local base = attacker:getBaseDmg() + fSTR + 
		(attacker:getStat(1) * 0.2 + attacker:getStat(6) * 0.2) * alpha; --D+fSTR+WSC
	...
	--Random num to work out if hit#6
	local double hexhit = math.random();
	local double hexhitc = math.random();
	if (hexhit<=hitrate) then
	--print("Hit.");
	if (hexhitc<=crithitrate) then
	--print("Critical Hit!");
	finaldmg = finaldmg + base*cpdif;
	--print("final damage so far: ",finaldmg);
	end
