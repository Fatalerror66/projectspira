...
-- 	Achantere TK  Northern Sandoria
--argument 1: guardnation: 01 is sandy,11 is Bastok and 21 is windy,31 and 41,etc automatically give you signet,other numbers with a different ones digit acts like other guardnation's outpost guards
-- the 3rd and 4th hex digit controls signet services, see conquest.lua.
--argument 2: expeditionary force bitmask
--argument 3: expeditionary force options: greater than a0 = you only need 5 members,greater than f0 = you only need 4,otherwise you still need 6.  less than a0 seems to be the status of the guardnation's control in the area
--- The lowest six bits encode the guardnation's conquest standing, see conquest.lua. 
...
if (player:getVar("supply_quest_started") > 0) and (player:getNation()==guardnation) and (supplies_fresh(player) == 0) then
	...
	event = Event(eventid);
	event:setParams(getArg1(guardnation, player),0,conquest_ranking(),getControlBitmask(guardnation),player:getVar("SUPPLY_QUESTS_SAN"),getArg6(player),getCP(player),0);
