-----------------------------------
--	Author: Tenjou & Arcanedemon
-- 	Windurst Border Guard
...
event0x7ff6 = Event(0x87ff6)
if (nation ~= 2) then
	event0x7ff6:setParams(0x10720,0,0x2d,0,0,0,0x15e,0);
end
player:startEvent(event0x7ff6);
end; 
	...
	if (nation ~= 2) then
		if (gil >= SetHPGil) then
			...
			if (Text == 0) then
				...
				player:showText(npc,1516);
			end
		else
			player:showText(npc,1517);
		end
	else
		player:setHomePoint();
