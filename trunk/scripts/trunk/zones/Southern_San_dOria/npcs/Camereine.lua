...
if (license == 1 and level >= 15) then
	...
	if (chocoquest >= (1 - CHOCOBO_RIDING_GAME_SANDORIA) and os.time() >= (chocoquesttime+604800)) then
		...
		event0x257:setParams(0xfffffffd,0,0,nation);
	else
		...
		event0x257:setParams(price,player:getCurrGil(),lvlblock); 
	end
	player:startEvent(event0x257);
else
...
if (count == 1 and item1) then
	...
	event0x257 = Event(0x257);
	event0x257:setParams(0xffffffff);
	player:startEvent(event0x257);
...
--print("CSID:",csid);
--print("OPTION:",option);
if (csid == 0x257 and option == 0) then
