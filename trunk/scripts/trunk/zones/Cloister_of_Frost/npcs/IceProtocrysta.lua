...
--	Author: Psyclosis 
--	Ice Protocrystal
--	Allows qualified players to enter the battlefield to fight Shiva Prime.
--
...
function onTrigger(player,npc)
	...
	if (npc:getXPos() >= 550 and npc:getXPos() <= 565 and npc:getZPos() >= 590 and npc:getZPos() <= 605) then --Entry crystal.
...
function onEventSelection(player,csid,option)
	...
	if (csid == 0x7d00 and option == 0) then
		record = getGlobalVar("TRIAL_BY_ICE_RECORD");
...
function onEventFinish(player,csid,option)
	...
	if (csid == 0x7D00 and option == 100) then --Enter the BC.
		...
		zone0xcb = Zone(0xcb);
		zone0xcb:spawnMob("ShivaPrime",500,42,-369,0x60,0);
		setGlobalVar("TRIAL_BY_ICE_ENTER_TIME",os.time());
	end	
end;