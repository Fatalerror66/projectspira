-----------------------------------
--	Author: Tenjou
-- 	BONEWORKERS' GUILDMASTER: Peshi Yohnts
-- 	Inducts players into the Bonecrafting Guild, and gives rank tests.
-----------------------------------
...
--print(rank,lvl);
member = player:getVar("isGuildMember-Bone");
item = 0;
text = 0;
test = player:getVar("takingTest-Bone");
if (member == 0) then
	--Offer to induct player.
	event0x2720:setParams(0,0,0,0,0,0,0,0);
else
	--See if the player needs a guild rank test.
	if (lvl >= (((rank+1)*10)-2) and rank ~= 9) then
		text = 0;
		player:setVar("takingTest-Bone",1);
		switch (rank) : caseof {
			[0] = function(x) item = 0x3482; end, --Recruit Test
			[1] = function(x) item = 0x3481; end, --Initiate Test
			[2] = function(x) item = 0x340b; end, --Novice Test
			[3] = function(x) item = 0x3493; end, --Apprentice Test
			[4] = function(x) item = 0x3323; end, --Journeyman Test
			[5] = function(x) item = 0x4393; end, --Craftsman Test
			[6] = function(x) item = 0x4024; end, --Artisan Test
			[7] = function(x) item = 0x30dc; end, --Adept Test
			[8] = function(x) item = 0x36a3; end, --Veteran Test
			--[9] = function(x) item = 0x3870; end, --Expert Test
...
event0x2720:setParams(item,0,text,member*254,0,0,0,0);
...
item0 = trade:hasItemQty(0x3482,1);
item1 = trade:hasItemQty(0x3481,1);
item2 = trade:hasItemQty(0x340b,1);
item3 = trade:hasItemQty(0x3493,1);
item4 = trade:hasItemQty(0x3323,1);
item5 = trade:hasItemQty(0x4393,1);
item6 = trade:hasItemQty(0x4024,1);
item7 = trade:hasItemQty(0x30dc,1);
item8 = trade:hasItemQty(0x36a3,1);
--item9 = trade:hasItemQty(0x3870,1);
...
if (count == 1 and item0 and rank == 0 and lvl >= 8 and test) then
	...
	trade:isComplete();
	player:setVar("takingTest-Bone",0);
	player:setSkillRank(54,1);
...
local bone = player:getVar("isGuildMember-Bone");
if (option == 1) then
	if (bone == 0) then
		...
		player:setVar("isGuildMember-Bone",1);
	end
end
end;