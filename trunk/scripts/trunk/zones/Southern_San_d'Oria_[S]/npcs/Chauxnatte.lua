...
if (license == 1 and level >= 20) then
	...
	--Dynamic pricing.
	timer = os.time();
	baseprice = CHOCOBO_PRICE_SANDORIA_S;
	lastprice = getGlobalVar("chocoboRentPrice-Sandoria-S");
	lasttime = getGlobalVar("chocoboRentTime-Sandoria-S");
	decay = CHOCOBO_PRICE_SANDORIA_S_DECAY;
	decaytime = CHOCOBO_PRICE_SANDORIA_S_DECAY_TIME;
	...
	setGlobalVar("chocoboRentOffer-Sandoria-S",price);
...
level = player:getMainLvl();
license = player:hasKeyItem(0x6aba);
...
if (count == 1 and item1 and level >= 20 and license) then
	...
	player:setPos(86,-62,274,0x40,0x51);
end
end;  
...
if (csid == 0x6a and option == 0) then
	flux = CHOCOBO_PRICE_SANDORIA_S_FLUX;
	price = getGlobalVar("chocoboRentOffer-Sandoria-S");
	player:removeGil(price);
	setGlobalVar("chocoboRentPrice-Sandoria-S",price+flux);
