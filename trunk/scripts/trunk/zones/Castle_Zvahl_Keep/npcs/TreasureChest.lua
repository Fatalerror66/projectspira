...
questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.263,5000,10000};
 treasureGemsArray = {0.184,Turquoise,Garnet,Goshenite,Peridot,Sphene,LightOpal};
treasureItemsArray = {0.553,PhalanxRing};
-----------------------------------
-- onTrigger Action
-----------------------------------
...
function onTrade(player,npc,trade)
 ...
 keyTraded = trade:hasItemQty(zoneKey,1);
  ...
  UnMoment = 0x153;
-- A Test of True Love 
if(player:getQuestStatus(1,62) == 1 and player:hasKeyItem(UnMoment) == 0) then
	totl = 1;
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			if( totl == 1) then
				player:addKeyItem(UnMoment);
				player:specialMessage(KEYITEM_OBTAINED,UnMoment);
			else
