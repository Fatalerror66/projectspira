-----------------------------------
--	Author: ReaperX
-- 	Lantern (NW one @ E-5 bottom floor, to Sacrificial Chamber)
...
if (count == 1 and item) then
  ...
  if (nw <= LANTERNS_STAY_LIT) then
    ...
    setGlobalVar("rancor_lantern_nw_last_lit",os.time());
    ...
    elseif (number_of_lit_lanterns == 4) then
      ...
      npc:setState(math.min(LANTERNS_STAY_LIT-ne,LANTERNS_STAY_LIT-sw,LANTERNS_STAY_LIT-se));       
    end;
  end
end
end; 
