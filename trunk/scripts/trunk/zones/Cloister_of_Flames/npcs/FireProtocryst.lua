...
--	Author: Psyclosis 
--	Fire Protocrystal
--	Allows qualified players to enter the battlefield to fight Ifrit Prime.
--
...
function onTrigger(player,npc)
	...
	if (npc:getXPos() >= -731 and npc:getXPos() <= -711 and npc:getZPos() >= -601 and npc:getZPos() <= -581) then --Entry crystal.
...
function onEventSelection(player,csid,option)
	...
	if (csid == 0x7d00 and option == 0) then
		record = getGlobalVar("TRIAL_BY_FIRE_RECORD");
...
function onEventFinish(player,csid,option)
	...
	if (csid == 0x7D00 and option == 100) then --Enter the BC.
		...
		zone0xcf = Zone(0xcf);
		zone0xcf:spawnMob("IfritPrime",500,42,-369,0x60,0);
		setGlobalVar("TRIAL_BY_FIRE_ENTER_TIME",os.time());
	end	
end;