...
-------------------------------------------
   treasureLVL = 65;
       ...
       zoneKey = 0x418; -- 1048
-- Loot
  treasureGilArray = {0.635,6300,26880};
 treasureGemsArray = {0.172,Aquamarine,Zircon,Chrysoberyl,Moonstone,Painite,Jadeite,Sunstone,Fluorite};
treasureItemsArray = {0.193,MagesBalladII};

--   AF related
    OldGauntlets = 0xD2;
 FightersCuisses = 0x3786;
MiquesPaintbrush = 0xDB; -- Key item for "Under Oath": http://wiki.ffxiclopedia.org/wiki/Under_Oath
      RoguesVest = 0x3163;
...
function onTrade(player,npc,trade)
	...
	UnderOath = 0; -- "Under Oath" Quest: http://wiki.ffxiclopedia.org/wiki/Under_Oath
   
	warlegs = 0;
	thfbody = 0;

	-- Checking if you can get the old gauntlets for BRD AF quest
	if    (player:getQuestStatus(3,53) == 1 and not(player:hasKeyItem(OldGauntlets) == 1)) then     							  -- BRD old gauntlets
		og = 1;
	elseif(player:getQuestStatus(0,92) == 1 and not(player:hasKeyItem(MiquesPaintbrush) == 1)) then                               -- MiquesPaintbrush
		UnderOath = 1;
	end
	-- Checking if you can get AF item after starting AF2 quest
	if player:getVar("KNOWS_ABOUT_BORGHERTZ") == 1 then
		if    (player:getQuestStatus(1,55) >= 1 and player:getMainJob() == 1 and not(player:hasItem(FightersCuisses) == 1)) then   -- WAR has completed AF2
			warlegs = 1;
		elseif(player:getQuestStatus(2,70) >= 1 and player:getMainJob() == 6 and not(player:hasItem(RoguesVest) == 1)) then        -- THF has completed AF2
			thfbody = 1;
		end
	end;
		
	if(og == 1 or UnderOath == 1 or warlegs == 1 or thfbody == 1)then
		questItemNeeded = 1;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			elseif(warlegs == 1) then
				-- WAR AF legs
				player:addItem(FightersCuisses);
				player:specialMessage(ITEM_OBTAINED,FightersCuisses);
			elseif(rdmbody == 1) then
				-- RDM AF body
				player:addItem(RoguesVest);
				player:specialMessage(ITEM_OBTAINED,RoguesVest);			
			elseif(UnderOath == 1) then
				-- MiquesPaintbrush
				player:addKeyItem(MiquesPaintbrush);
				player:specialMessage(KEYITEM_OBTAINED,MiquesPaintbrush);
			else
			    -- Not AF
