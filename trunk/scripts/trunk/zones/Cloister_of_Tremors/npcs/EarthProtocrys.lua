...
--	Author: Psyclosis 
--	Earth Protocrystal
--	Allows qualified players to enter the battlefield to fight Titan Prime.
--
...
function onTrigger(player,npc)
	...
	if (npc:getXPos() >= -548 and npc:getXPos() <= -528 and npc:getZPos() >= -505 and npc:getZPos() <= -485) then --Entry crystal.
...
function onEventSelection(player,csid,option)
	...
	if (csid == 0x7d00 and option == 0) then
		record = getGlobalVar("TRIAL_BY_EARTH_RECORD");
...
function onEventFinish(player,csid,option)
	...
	if (csid == 0x7D00 and option == 100) then --Enter the BC.
		...
		zone0xd1 = Zone(0xd1);
		zone0xd1:spawnMob("TitanPrime",500,42,-369,0x60,0);
		setGlobalVar("TRIAL_BY_EARTH_ENTER_TIME",os.time());
	end	
end;