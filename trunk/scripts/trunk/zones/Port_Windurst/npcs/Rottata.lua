...
--Rottata (Regional Teleporter)
--argument 1: regional control bitmask for Windurst
--argument 2: 
--argument 3: 
--argument 4: 
--argument 5: 
...
-- set player variable SUPPLY_QUESTS_WIN to 12582880 to give yourself all OPs including Tu'lia.
...
if (guardnation == player:getNation()) then
	event:setParams(control,0,0,0,0,0,player:getMainLvl(),0x3fffffff-player:getVar("SUPPLY_QUESTS_WIN"));
else
	event:setParams(0,0,0,0,0,0x201,player:getMainLvl(),0x3fffffff-player:getVar("SUPPLY_QUESTS_WIN"));
end;
