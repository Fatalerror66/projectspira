...
questItemNeeded = 0;
-- Loot
  treasureGilArray = {0.364,2450,7000};
 treasureGemsArray = {0.181,Amber,Amethyst,ClearTopaz,LapisLazuli,LightOpal,Onyx,Tourmaline};
treasureItemsArray = {0.455,EmethPick};
-----------------------------------
-- onTrigger Action
-----------------------------------
...
function onTrade(player,npc,trade)
	...
	GuidingBell = 0xE8;
	-- Wings of Gold 
	if(player:getQuestStatus(3,60) == 1 and player:hasKeyItem(GuidingBell) == 0) then
		wog = 1;
		questItemNeeded = wog;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			if(wog == 1) then
				player:addKeyItem(GuidingBell);
				player:specialMessage(KEYITEM_OBTAINED,GuidingBell);
			else
