...
-------------------------------------------
   treasureLVL = 66;
       ...
       zoneKey = 0x41C; -- 1052
-- Loot
  treasureGilArray = {0.875,4020,14400};
 treasureGemsArray = {0.125,Aquamarine,Zircon,Chrysoberyl,Moonstone,Painite,Peridot,Sunstone,Fluorite};
...
function onTrade(player,npc,trade)
	...
	ninhead = 0;
	-- Checking if you can get the old gauntlets for DRG AF quest
	if(player:getQuestStatus(3,57) == 1 and not(player:hasKeyItem(OldGauntlets) == 1)) then     -- DRG old gauntlets
		og = 1;
	end
	-- Checking if you can get AF item after starting AF2 quest
	if(player:getVar("KNOWS_ABOUT_BORGHERTZ") == 1 and player:getQuestStatus(5,144) >= 1 and player:getMainJob() == 13 and not(player:hasItem(NinjaHatsuburi) == 1)) then   -- NIN has completed AF2
		ninhead = 1;
	end

	if(og == 1 or ninhead == 1 or player:hasKeyItem(Map) == 0)then
		questItemNeeded = 1;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			elseif(ninhead == 1) then
				-- NIN AF head
				player:addItem(NinjaHatsuburi);
				player:specialMessage(ITEM_OBTAINED,NinjaHatsuburi);
			else
			    -- Not AF
