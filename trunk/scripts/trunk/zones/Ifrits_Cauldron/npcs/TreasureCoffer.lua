...
-------------------------------------------
   treasureLVL = 65;
       ...
       zoneKey = 0x41D; -- 1053
-- Loot
  treasureGilArray = {0.929,7200,21060};
 treasureGemsArray = {0.071,Aquamarine,Chrysoberyl,Moonstone,Painite,Sunstone,Fluorite};
...
function onTrade(player,npc,trade)
	...
	drgbody = 0;
	-- Checking if you can get the old gauntlets for NIN AF quest
	if(player:getQuestStatus(3,56) == 1 and not(player:hasKeyItem(OldGauntlets) == 1)) then
		og = 1;
	end
	-- Checking if you can get AF item after opening AF2 quest
	if(player:getVar("KNOWS_ABOUT_BORGHERTZ") == 1 and player:getQuestStatus(0,95) >= 1 and not(player:hasItem(DrachenMail) == 1)) then -- DRG has completed AF2
		drgbody = 1;
	end

	if(og == 1 or drgbody == 1 or player:hasKeyItem(Map) == 0)then
		questItemNeeded = 1;
	end
...
if (( keyTraded or isTHFKey(trade) ) and count == 1 and gil == 0) then  -- Player traded a key and zero gil.
	...
	if(not(success == -2)) then
		...
		if (diceroll <= success) then
			...
			elseif(drgbody == 1) then
				-- DRG AF body
				player:addItem(DrachenMail);
				player:specialMessage(ITEM_OBTAINED,DrachenMail);
			else
			    -- Not AF
