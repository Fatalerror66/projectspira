...
--
--	Tenjou: Edited this a little bit during testing.  Tools are removed from spellcast.lua.
--------------------------------------
-- Includes
...
function OnSpellCast(caster,target,spell)
	...
	if(blink ~= nil)then
		...
		if (utsusemi ~= nil and utsusemi:getPower() > power) then -- Utsusemi: Ni or San already up?
			spell:setMsg(75); -- Ichi won't overwrite Ni or San shadows
		elseif(utsusemi ~= nil and utsusemi:getPower() == power)then -- Utsusemi: Ichi overwrite
			target:removeStatusType(EFFECT_UTSUSEMI);
			target:addStatusEffect(EFFECT_UTSUSEMI,power,0,duration);
			spell:setMsg(230);
		else -- No Utsusemi effect on player. Add it!
			target:addStatusEffect(EFFECT_UTSUSEMI,power,0,duration);
			spell:setMsg(230);
		end
  	end  					
