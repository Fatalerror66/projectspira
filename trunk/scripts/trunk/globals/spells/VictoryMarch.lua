...
--	Author: Tenjou
-- 	Victory March
-- 	Increases attack speed for party members within the area of effect.
--
--	http://wiki.ffxiclopedia.org/wiki/March
--------------------------------------
-- Includes
...
function OnSpellCast(caster,target,spell)
	...
	-- Defines
	name = EFFECT_MARCH; --Actually is type, but type is a keyword in LUA.
	tic = 0;
	duration = 120;
	...
	power = 2 + math.floor(caster:getSkill(SINGING_SKILL)/32);
	...
	if (instrument == Gjallarhorn or (instrument == 18341 and isInDynamis(caster))) then
		power = power + 2;
	elseif (instrument == 17367 or instrument == 17835 or instrument == 17836) then --Royal Spearman's Horn, San d'Orian Horn, or Kingdom Horn
		power = power + 1;
	elseif (instrument == 17349 or instrument == 17853) then --Faerie Piccolo or Iron Ram Horn
		power = power + 2;
	end
