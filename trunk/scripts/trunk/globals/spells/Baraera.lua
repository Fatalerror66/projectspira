...
--	Author: Tenjou
-- 	Baraera
-- 	Increases caster's resistance against wind.  Afflatus Solace: Grants MDB against wind.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARFIRE,EFFECT_BARWATER do
		...
		if (barelement ~= nil and (BARELEMENT_OVERWRITE == 1 or (BARELEMENT_OVERWRITE == 0 and i == EFFECT_BARAERO))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARAERO,resist,0,duration,FLAG_DISPELABLE);
	target:updateEnmity(caster,80,1);
	final = EFFECT_BARAERO;
