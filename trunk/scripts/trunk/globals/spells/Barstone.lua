...
--	Author: Tenjou
-- 	Barstone
-- 	Increases caster's resistance against earth.  Afflatus Solace: Grants MDB against earth.
--
...
function OnSpellCast(caster,target,spell)
	...
	for i = EFFECT_BARFIRE,EFFECT_BARWATER do
		...
		if (barelement ~= nil and (BARELEMENT_OVERWRITE == 1 or (BARELEMENT_OVERWRITE == 0 and i == EFFECT_BARSTONE))) then
			target:removeStatusType(i);
		end
	end
	
	--Do it!
	target:addStatusEffect(EFFECT_BARSTONE,resist,0,duration,FLAG_DISPELABLE);
